...
class WordFile():
	...
	def add_orientations(self):
		for orientation_status in self.orientations.keys():
			self.add_subsection(orientation_status)

			for orientation_type in self.orientations[orientation_status]:
				self.document.add_heading(orientation_type, 1)  # Add the key as a title

				self.add_numbered_table(self.orientations[orientation_status][orientation_type])
